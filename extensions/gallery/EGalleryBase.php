...
class EGalleryBase extends CWidget {
	...
	protected function getAlbums()
	{
		...
		if($_cache!==false && json_decode($_cache, true)!==null):
			...
			foreach ($albums as $album) {
				if ($album->isDir() && $album != "thumbs" && !$album->isDot() && substr($album->getFilename(), 0, 1) != '.') {
	...
	protected function getImages($path, $single = false)
	{
		...
		if($_cache!==false && json_decode($_cache, true)!==null):
			...
			//{
				//if(Yii::app()->request->pathInfo == "sGallery/list") {
					$this->path =Yii::app()->request->baseUrl."/shareimages/photo";
				//} else
				//	$this->path =Yii::app()->request->baseUrl."/".$this->path;
			//}
