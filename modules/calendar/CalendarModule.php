...
class CalendarModule extends CWebModule
{
	...
	public $calendarOptions = array(
			...
			// theme place in folder 'calendar/component/fullCal'
			//'themeName'=>'redmond',
			'themeName'=>'2jui-bootstrap',
			...
			// Call controller 'calendar/cron' every 'cronPeriod' minutes.
	...
	protected function tableCreate()
	{
		...
		if($db)
		{
			...
			{
				$sql = "CREATE TABLE IF NOT EXISTS `s_events` (
			...
			{
				$sql = "CREATE TABLE IF NOT EXISTS `s_events_helper` (
				`id` int unsigned NOT NULL auto_increment,
				`user_id` int unsigned NOT NULL,
				`title` varchar(1000) character set utf8 default NULL,
				PRIMARY KEY  (`id`)
			...
			{
				$sql = "CREATE TABLE IF NOT EXISTS `s_events_user_preference` (
				`user_id` int unsigned NOT NULL,
