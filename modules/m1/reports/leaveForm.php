...
class leaveForm extends fpdf
{
	...
	function report($model)
	{
		...
		$this->SetFont('Arial','',10);
		$this->Cell(40,8,'NRK');
		$this->Cell(60,8,':  '.$model->person->employee_code);
		$this->Cell(0,8,'','R');
		$this->Ln();
		...
		if (time() >= strtotime($model->person->companyfirst->start_date."+1 year")) {
			if ($model->person->leaveBalance->balance >= 12) {
				$cJoinDate = (int)$model->person->leaveBalance->balance;
			} else 
				$cJoinDate = 12;
		} else 
			$cJoinDate = 0;
		...
		$this->Cell(110,4,'            1. Hutang Cuti Periode sebelumnnya','L');
		$this->Cell(5,4,': ');
