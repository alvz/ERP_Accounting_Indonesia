...
{	
	...
	function report($id)  
	{
		...
		$model=gPerson::model()->with('many_career','many_status',
		'many_experience','many_education',
		'many_educationnf','many_family')->findByPk((int)$id);
		...
		$this->SetFont('Arial','',10);
		$this->Cell(100,5,$model->employee_code);
		$this->Ln();
		$this->Cell(30);
		$this->SetFont('Arial','B',9);
		...
		$this->SetFont('Arial','',10);
		$this->Cell(100,5,$model->home_phone.' / '.$model->handphone.' / '.$model->handphone2);
		$this->Ln();
		$this->Cell(30);
		$this->SetFont('Arial','B',9);
		$this->Cell(35,5,'Email');
		$this->SetFont('Arial','',10);
		$this->Cell(100,5,$model->email.' ; '.$model->email2);
		$this->Ln();
		$this->Cell(30);
		$this->SetFont('Arial','B',9);
		...
		foreach($model->many_experience as $mod_experience)
		{
			...
			$this->Ln();
			$this->Cell($w[0],5,'');
			$this->Cell($w[1]+$w[2],5,' ('.$mod_experience->industries.')');
		...
		$_counter = 1;
		$_countert = 1;
		$w=array(8,20,40,20,20,25,35,22);
		...
		foreach($model->many_educationnf as $mod_educationnf)
		{
			$this->SetFont('Arial','',10);
			$this->Cell($w[0],5,number_format($_countert,0,',','.'),0,0,'R');
			$this->Cell($w[1],5,$mod_educationnf->education_name);
