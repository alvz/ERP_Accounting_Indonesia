...
class HVacancyController extends Controller
{
	...
	public function actionIndex()
	{
		...
		if(isset($_GET['hVacancy'])) {
			...
			$criteria1->compare('vacancytitle',$_GET['hVacancy']['vacancytitle'],true,'OR');
	...
	public function actionEmail($id) {
		...
		if(isset($_POST['fInvitation']))
		{
			...
			if($model->validate()) {
				...
				$subject="Interview Invitation : ".$modelVA->vacancy->vacancytitle." at ".$modelVA->vacancy->company->name;
				
				$body=	"Dear ".$modelVA->applicant->candidate_name.","."\r\n\r\n\r\n".
						"We are invite you for selection process (Interview) for ".$modelVA->vacancy->vacancytitle." at ".$modelVA->vacancy->company->name. "will be held on:"."\r\n\r\n\r\n".
	...
	{
		$model=hVacancy::model()->findByPk($id);
		if($model===null)
	...
	public function actionCreate()
	{
		...
		if(isset($_POST['hVacancy']))
		{
			...
			$mode->timestamp=time();
			$model->status_id=2; //2. Open Job.  Directly OPENED..
	...
	public function actionToExcel($id) {
		...
		$foo->setCellValue("A1", $title[0]->vacancy->vacancytitle)->getStyle("A1:H1")->applyFromArray($styleBackground);
	...
	public function actionBroadcast($id)
	{
		...
		Title: ".$modelVacancy->vacancytitle."<br/>
