...
class gSelection extends BaseModel
{
	 ...
	 */
	public function search($id,$bss=false)
	{
	...
	public function searchBss($id)
	{
		...
		switch ($id) {
		case 'newentry':
			$criteria->condition='invitation_date is null and workflow_result_id = 1';
			break;
		case 'invited':
			$criteria->condition='
			invitation_date is not null
			and interviewhr1_date is null
			and interviewhr2_date is null
			and interviewuser1_date is null
			and interviewuser2_date is null
			and interviewuser3_date is null
			and psikotest_date is null
			and final_date is null';
			break;
		case 'appointment':
			$criteria->condition='invitation_date = "'.date("Y-m-d") .'" and final_date is null';
			break;
		case 'appointment1':
			$criteria->condition='invitation_date = "'.date("Y-m-d",strtotime(date("Y-m-d")."+1 day")) .'" and final_date is null';
			break;
		case 'appointment2':
			$criteria->condition='invitation_date = "'.date("Y-m-d",strtotime(date("Y-m-d")."+2 day")) .'" and final_date is null';
			break;
		case 'appointment3':
			$criteria->condition='invitation_date = "'.date("Y-m-d",strtotime(date("Y-m-d")."+3 day")) .'" and final_date is null';
			break;
		case 'psikotestschedule':
			$criteria->condition='
			psikotest_date is not null 
			and psikotest_result_id is null 
			and final_date is null';
			$criteria->order='psikotest_date DESC';
			break;
		case 'psikotestresult':
			$criteria->condition='
			psikotest_date is not null 
			and psikotest_result_id is not null 
			and final_date is null';
			$criteria->order='psikotest_date DESC';
			break;
		case 'interviewhr':
			$criteria->condition='
			(interviewhr1_date is not null 
			OR interviewhr2_date is not null) 
			and final_date is null';
			$criteria->order='interviewhr1_date DESC,interviewhr2_date DESC';
			break;
		case 'interviewuser':
			$criteria->condition='
			(interviewuser1_date is not null 
			OR interviewuser2_date is not null 
			OR interviewuser3_date is not null) 
			and final_date is null';
			$criteria->order='interviewuser1_date DESC,interviewuser2_date DESC,interviewuser3_date DESC';
			break;
		default:
			$criteria->condition='final_date is not null';
			$criteria->order='final_date DESC';
		}
		
		$criteria1=new CDbCriteria;
		$criteria1->compare('company_id',sUser::model()->getGroup());
