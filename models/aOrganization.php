...
class aOrganization extends BaseModel
{
	...
	public function rules()
	{
		return array(
				array('name', 'required'),
				array('kabupaten_id, propinsi_id, created_date, created_by, updated_date, updated_by, parent_id', 'numerical', 'integerOnly'=>true),
				array('branch_code, name, address2, address3, address4, pos_code, phone_code_area, telephone, fax, email, website', 'length', 'max'=>50),					
				array('address', 'length', 'max'=>300),
				array('photo_path', 'length', 'max'=>100),
				array('id, branch_code, name, address, address2, address3, address4, pos_code, phone_code_area, telephone, fax, email, website, created_date, created_by, updated_date, updated_by', 'safe', 'on'=>'search'),
		);
	}
	...
	public function getPhotoPath() {
		if ($this->photo_path != null && $this->PhotoExist) {
			if ($this->PhotoExistThumb) {
				...
				$path=CHtml::image(Yii::app()->request->baseUrl . "/shareimages/company/FA-logo-APL-1_ONLY.jpg", CHtml::encode($this->name), array("width"=>"100%",'id'=>'photo'));
		}
		return $path;
